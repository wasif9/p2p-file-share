...
## Run with Docker
1. (Optional) Create a ``.env`` file in the bootstrap folder contains\
``BOOTSTRAP_PRIVKEY=<based64 Ed25519 pirvate key>``
2. ``cd ..`` to go to the root directory of the project since we need to copy ``go.mod`` and ``go.sum``.
3. ``docker build -t bootstrap -f .\bootstrap\Dockerfile .`` to build the image.
4. ``docker run -p 4001:4001 --name bootstrap bootstrap`` to build and run the container.
5. Crate an ``.env`` environmnet configuration file under the ``peer-app`` directory.
6. Add the following contents to the ``.env`` file\
